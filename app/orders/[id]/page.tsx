...
export default function OrderDetailPage() {
  ...
  // ✅ FIX — polling berhenti ketika status sudah terupdate
  useEffect(() => {
    ...
    const fetchOrderData = async () => {
      try {
        ...
        if (updatedOrder.payment) setPayment(updatedOrder.payment);

        if (paymentStatus === "success") {
          const isPaymentDone = updatedOrder.payment?.status === "paid";
          const isOrderProcessing = updatedOrder.status === "processing";

          if (isPaymentDone || isOrderProcessing) {
    ...
    if (paymentStatus === "success") {
      ...
      const startPolling = async () => {
        const needsPolling = await fetchOrderData();
        ...
        const interval = setInterval(async () => {
          ...
          if (attempts >= maxAttempts) {
            clearInterval(interval);
            setPolling(false);
            toast("Pembayaran sedang diverifikasi", { icon: "ℹ️" });
          }
        }, 3000);
      };
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "delivered":
        return <FiCheck className="text-lime-600" />;
      case "cancelled":
        return <FiX className="text-red-600" />;
      default:
        return <FiShoppingBag className="text-neutral-400" />;
    }
  };
            ...
            <FadeIn>
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex items-center gap-4">
                  <span
                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border ${getStatusColor(order.status)}`}
                  >
                  ...
                  {payment && payment.status === "paid" && (
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium bg-lime-100 text-lime-700 border border-lime-200">
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-neutral-200 p-6">
                    ...
                    {order.notes && (
                      <div className="border-t border-neutral-200 mt-6 pt-6">
                        ...
                        <p className="text-neutral-600 bg-neutral-50 p-4 rounded-xl">
                          {order.notes}
                        </p>
                      </div>
                    )}
                    ...
                    {!payment ? (
                      <div className="text-center py-4">
                        ...
                        <button
                          ...
                          className="w-full bg-lime-600 text-white py-3 rounded-xl font-medium hover:bg-lime-700 disabled:bg-neutral-300 transition"
                        >
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex items-center gap-2">
                            {polling && (
                              <svg className="animate-spin h-4 w-4 text-lime-600" viewBox="0 0 24 24">
                                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none" />
                                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                              </svg>
                            )}
                            <span
                              className={`px-3 py-1 rounded-full text-xs font-medium border ${getPaymentStatusColor(payment.status)}`}
                            >
                              {payment.status}
                            </span>
                          </div>
                        ...
                        {payment.status === "paid" && (
                          <div className="pt-4 border-t">
                            <div className="bg-lime-50 text-lime-700 p-4 rounded-xl text-center font-medium">
                              ✓ Payment Complete
                            </div>
                          </div>
                        )}
